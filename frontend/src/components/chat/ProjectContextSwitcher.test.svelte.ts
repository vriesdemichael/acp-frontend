...
import ProjectContextSwitcher from './ProjectContextSwitcher.svelte'
...
const DEFAULT_PROPS = {
  ...
  onRemoveProject: vi.fn<(id: string) => Promise<void>>().mockResolvedValue(undefined),
  onAddProject: vi
...
function renderSwitcher(overrides: Partial<typeof DEFAULT_PROPS> = {}) {
  return render(ProjectContextSwitcher, { props: { ...DEFAULT_PROPS, ...overrides } })
}
...
describe('ProjectContextSwitcher', () => {
  ...
  it('preserves draft values when toggling the add form', async () => {
    ...
    fireEvent.input(screen.getByRole('combobox', { name: /Project path/i }), {
      target: { value: '/tmp/project-draft' },
    })

    fireEvent.click(screen.getByRole('button', { name: /Hide Add Project/i }))
    await waitFor(() => screen.getByRole('button', { name: /^Add Project$/i }))
  ...
  it('renders a scrollable path suggestion list', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Open/i }))
    fireEvent.click(await screen.findByRole('button', { name: /Add Project/i }))
    await waitFor(() => screen.getByRole('combobox', { name: /Project path/i }))
    fireEvent.input(screen.getByRole('combobox', { name: /Project path/i }), {
      target: { value: '/work' },
    })
