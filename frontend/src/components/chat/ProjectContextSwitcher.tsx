...
export function ProjectContextSwitcher({
      ...
      >
        ...
        {pathSuggestionsOpen && pathSuggestions.length > 0 ? (
          <div
            ...
            className="max-h-72 overflow-y-auto rounded-xl border border-teal-500/20 bg-[linear-gradient(180deg,rgba(8,15,24,0.98),rgba(5,10,18,0.98))] p-2 shadow-[0_18px_40px_rgba(2,6,23,0.4)]"
          >
  ...
  return (
    <>
      ...
      {managerOpen && addFormOpen && isCompactViewport && typeof document !== 'undefined'
        ? createPortal(
            <div className="fixed inset-0 z-[60] bg-slate-950/82 px-3 py-3 backdrop-blur-sm sm:px-6 sm:py-6">
              <div
                data-testid="mobile-add-project-modal"
                className="mx-auto flex h-full w-full max-w-2xl flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#0a0f17] shadow-[0_30px_120px_rgba(2,6,23,0.75)]"
              >
                ...
                <section className="min-h-0 flex-1 overflow-y-auto px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:px-5">
                  {addProjectForm}
                </section>
              </div>
            </div>,
